...
describe("render geometry", () => {
  it("resolves the selected social preset", () => {
    expect(outputDimensions(900, 1600, { resizeMode: "fixed", presetId: "pinterest-pin", customWidth: 900, customHeight: 900, fitMode: "contain", format: "jpeg", jpegQuality: 92 })).toEqual({ width: 1000, height: 1500 });
  });

  it("can preserve source dimensions", () => {
    expect(outputDimensions(2048, 1365, { resizeMode: "original", presetId: "xiaohongshu-portrait", customWidth: 1600, customHeight: 1200, fitMode: "contain", format: "png", jpegQuality: 92 })).toEqual({ width: 2048, height: 1365 });
  });

  it("supports arbitrary custom output dimensions within the Canvas limit", () => {
    expect(outputDimensions(2048, 1365, { resizeMode: "custom", presetId: "tiktok-vertical", customWidth: 1080, customHeight: 1920, fitMode: "cover", format: "png", jpegQuality: 92 })).toEqual({ width: 1080, height: 1920 });
